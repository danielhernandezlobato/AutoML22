...
DEFAULT_CONSTRAINT_DELTA = 0.1  # for now we have one for each constraint
...
def parse_config_file(config_file_dir, config_file_name, verbose=True):
    ...
    if numObjectives > 1 and not (options["acquisition"] in set(['PESM','ParEGO', 'RANDOM', 'EHI', 'SUR', 'SMSego', 'PESMC', 'MESMC', 'BMOO'])):
        raise Exception("You have %d objectives (i.e. a multi-objective problem) and an acquisition function different \
			from EHI, PESM, PESMC, MESMC, ParEGO, SUR or SMSego." \
		% numObjectives)
